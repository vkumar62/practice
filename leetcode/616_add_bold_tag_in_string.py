from collections import namedtuple

SubStringOffset = namedtuple("SubStringOffset", ["start", "end"])

import pdb

class Solution:
    def addBoldTag(self, s, dic):
        """
        :type s: str
        :type dict: List[str]
        :rtype: str
        """
#        pdb.set_trace()
        max_dict_len = max(map(len, dic))
        dict_strs = dict(((x, len(x)) for x in dic))
        
        i = 0
        offsets = []
        for i in range(len(s)):
            for j in range(i+1, min(len(s)+1, i+max_dict_len+1)):
                # Check if substring i:j is in the dict
                if s[i:j] in dict_strs:
                    offsets.append(SubStringOffset(i,j))
        
        if not offsets:
            # No strings have substring
            return s
        
        # Now we have all the offsets. Add the breakers appropriately
        # Do a union on offsets:
        union_offsets = [offsets[0]]
        for offset in offsets[1:]:
            if offset.start <= union_offsets[-1].end:
                union_offsets[-1] = SubStringOffset(union_offsets[-1].start, max(offset.end, union_offsets[-1].end))
            else:
                union_offsets.append(offset)
        
        result = []
        cur_offset = 0
        for i, c in enumerate(s):
            if cur_offset < len(union_offsets):
                if i == union_offsets[cur_offset].start:
                    result.append('<b>')
                elif i == union_offsets[cur_offset].end:
                    result.append('</b>')
                    cur_offset += 1
            result.append(c)
        if union_offsets[-1].end == len(s):
            result.append('</b>')
        return ''.join(result)


class Solution:
    def addBoldTag(self, s, dic):
        """
        :type s: str
        :type dict: List[str]
        :rtype: str
        """
#        pdb.set_trace()
        max_dict_len = max(map(len, dic))
        dict_strs = set(dic)

        offsets = []
        for i in range(len(s)):
            end_offset = min(len(s), i+max_dict_len)
            start_offset = max(i+1, offsets[-1].end) if offsets else i + 1
            for j in reversed(range(start_offset, end_offset+1)):
                if s[i:j] in dict_strs:
                    if offsets and i <= offsets[-1].end:
                        offsets[-1] = SubStringOffset(offsets[-1].start, max(offsets[-1].end, j))
                    else:
                        offsets.append(SubStringOffset(i, j))
                    break

            if offsets and offsets[-1].end >= len(s):
                break

        result = []
        cur_offset = 0
        for i, c in enumerate(s):
            if cur_offset < len(offsets):
                if i == offsets[cur_offset].start:
                    result.append('<b>')
                elif i == offsets[cur_offset].end:
                    result.append('</b>')
                    cur_offset += 1
            result.append(c)
        if offsets[-1].end >= len(s):
            result.append('</b>')
        return ''.join(result)

class Solution:
    def addBoldTag(self, s, dic):
        """
        :type s: str
        :type dict: List[str]
        :rtype: str
        """
#        pdb.set_trace()
        if not s or not dic:
            return s

        max_dict_len = max(map(len, dic))
        dict_strs = set(dic)

        offsets = []
        for i in range(len(s)):
            for word in dic:
                if s[i:].startswith(word):
                    j = i + len(word)
                    if offsets and i <= offsets[-1].end:
                        offsets[-1] = SubStringOffset(offsets[-1].start, max(offsets[-1].end, j))
                    else:
                        offsets.append(SubStringOffset(i, j))
                    break

            if offsets and offsets[-1].end >= len(s):
                break

        result = []
        prev_end = 0
        for offset in offsets:
            result.append(s[prev_end:offset.start])
            result.append('<b>')
            result.append(s[offset.start:offset.end])
            result.append('</b>')
            prev_end = offset.end
        result.append(s[prev_end:])
        return ''.join(result)

s = "abcxyz123"
dic = ["abc","123"]
print(Solution().addBoldTag(s, dic))
s = "aaabbcc"
dic = ["aaa","aab","bc"]
print(Solution().addBoldTag(s, dic))

#pdb.set_trace()
s = ""
dic = ["QH9sUl","","JfeicLOEtfTBA1D5RFvsley0J2Ut8l","WELTdMnQzTHEaXDfiljZaufDYMXXu2Vk7QUKHNbj0ZTP9NP5dV10zAdv8B7xkLlwOR8yeTxTdAeInR8s1ZDjq8XM77Ru8RRdYEHN2XPorzCMjwngVM0mbKJ8Gq0XVmrw","RFlVw1HmmyVDWilgdFjhnDjOdq6jZAJOn8OrGy61np2yAqPS5328Y4cuHRS7BDTMMGxJBThj8aXqKXBywJlYcldVV4hX67lNUYqZXgbSvR2cdvYrGy","","L2NIjfp8jjHrMqLaAtEJUC7Kc4uxnAc9K8fKF1wNmA31mAYHryp73","1nE6Bb2QOeRS87RbfbHrIpjecaH2cVAmPhNThk8raF7Yn0aM9uDZa0Ij5s3BkpsOgHtYan34bchbeh7i8n3AiyiSPFWgAwzbmQX9rKqPZnCbJL5wc2doAHDQhr5jAGGedLKQdQMH9pRfaKeVUr8Qwhm6WP5HAqd1lqsmzaMyh","t8m1fH68PxUFl5yJJRi31PA2OVlVpuE5tk4mU8dqP6NfJfeicLOEtfTBA1D5RFvsley0J2U","nOkqMLkPvsnlzqRkmPp7Bip81gNhLyUhXCCZGEQQHTiRa8KWxKxoLVbBfrzN3PEwd11U","J34MjYDMP1QH9sU","","04Srn4BksGIueQVsnwdIBnLf5PtSK4jGE82T6T4sk06DIeJtiwsKlhjhMajQOypjSrRo0wEVwO0o0iDwqM7wbe0VvSXaJ3J34MjYDMP1QH9sUla9zFt9EFWQMcwGH","sKlhjhMajQ","eQVsnwdIBnLf5PtSK4jGE82T6T4sk06DIeJtiwsKlhjhMajQOypjSrRo0wEVwO0o0iDwqM7wbe0VvSXaJ3J34MjYDMP1QH9sUla9zFt9EFWQMcwGHik0ANzrPXr3wJlNvmr6BBxFutVShEucJykX12S1ni1Oby7anyhm5wGZruXyBqmht8m1fH68PxUFl5yJJRi31PA2OVlVpuE5tk4mU8dqP6NfJfeicLO","","","w5u3pqcyVfoSt6w","","S1ni1Oby7anyhm5wGZruXyBqmht8m1fH68PxUFl","","ht8m1fH68PxUFl5yJJRi31PA2OVlVpuE5tk4mU8dqP6NfJfeicLOEtfTBA1D5RFvsley0J2Ut8lAaZ","","","KlhjhMajQOypjSrRo0wEVwO0o0iDwqM7wbe0VvSXaJ3J34MjYDMP1QH9sUla9zFt9EFWQMcwGHik0ANzrPXr3wJlNvmr6BBxFutVShEucJykX12S1ni1Oby7anyhm5wGZruXyBqmht8m1fH68PxUFl5yJJRi31PA2OVlVpu","","wiBlClPhKM9uGfbFSqXWELTdMnQzTHEaXDfiljZaufDYMXXu2Vk7QUKHNbj0ZTP9NP5dV10zAdv8B7xkLlwOR8yeTxTdAeInR8s1ZDjq8XM77Ru8RRdYEHN2XPorzCMjwngVM0mbKJ8Gq0XVmrwC7Na3Uvtb2DqU04Srn4Bks","4BksGIueQVsnwdIBnLf5PtSK4jGE82T6T4sk06DIeJtiwsKlhjhMajQOypjSrRo0wEVwO0o0iDwqM7wbe0VvSXaJ3J34MjYDMP1QH9sUla9zFt9EFWQMcwGHik0ANzrPXr3wJlNvmr"]

expected = "9NLRDk5b4igx8bZGb8wJayqENXuksNK0kIdI6<b></b>"

mine = Solution().addBoldTag(s,dic)
if mine != expected:
    pdb.set_trace()
